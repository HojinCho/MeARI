...
from ..utils.types cimport func_t_double_par, func_t_double2_par, func_t_param_dblptr2
...
ctypedef void (*func_prhq_quadrature)(packet_PRHQ *) noexcept nogil
ctypedef void (*func_prhq_quadrature)(packet_PRHQ *) noexcept nogil
ctypedef struct bundle_quadrature:
    # Global buffer
    long nijsym   # = ndata*(ndata+1)//2
    long npqsym   # = nresps*(nresps+1)//2
    long n_quad   # = ntau?
    double * t_quad     # Quadrature tau values
    double * w_quad     # Quadrature weights
    double * Psi_p_quad # Psi values
    func_t_double_par acf1
    double * kernel_p
    # Computing Functions
    func_prhq_quadrature interpolate
    func_prhq_quadrature prep
    func_prhq_quadrature compute_covmat


cdef void quadrature_set_tf_copy(packet_PRHQ * packet) noexcept nogil: 
    cdef bundle_quadrature * bundle = (<bundle_quadrature *>packet.bundle)
    # Intended for the case where sampling of tf is already sampled at quadratures.
    memcpy(bundle.Psi_p_quad, packet.tf, packet.nresp*packet.ntau*sizeof(double))

cdef void quadrature_set_tf_interpolate(packet_PRHQ * packet) noexcept nogil:
    cdef double * t_rb = <double *> malloc((nx-4)  *sizeof(double))
...
cdef void covmat_ij_quadrature_prep_has_driv(packet_PRHQ * packet) noexcept nogil:
    pass


##### Need to fix next lines.
cdef void covmat_ij_quadrature_prep_DRW_has_driv(packet_PRHQ * packet) noexcept nogil:
    cdef bundle_quadrature * bundle = (<bundle_quadrature *>packet.bundle)
...
cdef void covmat_ij_quadrature_prep_DRW_no_driv(packet_PRHQ * packet) noexcept nogil:
    cdef bundle_quadrature * bundle = (<bundle_quadrature *>packet.bundle)
    # called once every loglike call
...
cdef double quadrature_l( # use when -dt < bound_l
...
cdef double quadrature_r( # use when -dt > bound_r
    double dt_ij, double bound_l, double bound_r, double invtau_d,
    double sum_L, double sum_R,
) noexcept nogil: # invtau_d = 1/tau_d
    return sum_L*exp((dt_ij - bound_l)*invtau_d)

cdef double quadrature_c(
    double dt_ij, double bound_l, double bound_r, double invtau_d,
...
cdef void compute_covmat_has_driv(packet_PRHQ * p) noexcept nogil:
    # printf("YES DRIV!\n")
    # C is not packed.
    cdef bundle_quadrature * b = (<bundle_quadrature *>p.bundle)
...
cdef int initialize_PRHQ_quadrature(packet_PRHQ * p) noexcept nogil:
    cdef:
        long n_ccf = 2*p.ntau - 1
        long nijsym = p.ndata*(p.ndata+1)//2
        long npqsym = p.nresp*(p.nresp+1)//2
    # Method-specific buffers
    p.bundle = <void *> malloc(sizeof(bundle_quadrature))
    (<bundle_quadrature *>p.bundle).n_ccf = n_ccf
    (<bundle_quadrature *>p.bundle).nijsym = nijsym
    (<bundle_quadrature *>p.bundle).npqsym = npqsym
    (<bundle_quadrature *>p.bundle).t_cf   = <double *> malloc(n_ccf * sizeof(double))
    if p.lc.has_driving:
        (<bundle_quadrature *>p.bundle).F_p_L = <double *> malloc((p.nresp * p.ntau) * sizeof(double))
        (<bundle_quadrature *>p.bundle).F_p_R = <double *> malloc((p.nresp * p.ntau) * sizeof(double))
        (<bundle_quadrature *>p.bundle).U_p_L = <double *> malloc((p.nresp         ) * sizeof(double))
        (<bundle_quadrature *>p.bundle).U_p_R = <double *> malloc((p.nresp         ) * sizeof(double))
        (<bundle_quadrature *>p.bundle).prep           = covmat_ij_quadrature_prep_DRW_has_driv
        (<bundle_quadrature *>p.bundle).compute_covmat = compute_covmat_has_driv
    else:
        (<bundle_quadrature *>p.bundle).F_p_L = NULL
        (<bundle_quadrature *>p.bundle).F_p_R = NULL
        (<bundle_quadrature *>p.bundle).U_p_L = NULL
        (<bundle_quadrature *>p.bundle).U_p_R = NULL
        (<bundle_quadrature *>p.bundle).prep           = covmat_ij_quadrature_prep_DRW_no_driv
        (<bundle_quadrature *>p.bundle).compute_covmat = compute_covmat_no_driv
    (<bundle_quadrature *>p.bundle).G_pq_L = <double *> malloc((npqsym * n_ccf) * sizeof(double))
    (<bundle_quadrature *>p.bundle).G_pq_R = <double *> malloc((npqsym * n_ccf) * sizeof(double))
    (<bundle_quadrature *>p.bundle).V_pq_L = <double *> malloc((npqsym        ) * sizeof(double))
    (<bundle_quadrature *>p.bundle).V_pq_R = <double *> malloc((npqsym        ) * sizeof(double))
    if p.var == VarType.DRW:
        (<bundle_quadrature *>p.bundle).acf1 = var_DRW_oneside
    elif p.var == VarType.DRW_FAST:
        (<bundle_quadrature *>p.bundle).acf1 = var_DRW_fast_oneside
    elif p.var == VarType.LOG_DRW:
        (<bundle_quadrature *>p.bundle).acf1 = var_LOG_DRW_oneside
    elif p.var == VarType.LOG_DRW_FAST:
        (<bundle_quadrature *>p.bundle).acf1 = var_LOG_DRW_fast_oneside

    if (
        (<bundle_quadrature *>p.bundle).t_cf == NULL 
        or (<bundle_quadrature *>p.bundle).G_pq_L == NULL or (<bundle_quadrature *>p.bundle).G_pq_R == NULL
        or (<bundle_quadrature *>p.bundle).V_pq_L == NULL or (<bundle_quadrature *>p.bundle).V_pq_R == NULL
    ):
        return -1
    if p.lc.has_driving and (
           (<bundle_quadrature *>p.bundle).F_p_L == NULL or (<bundle_quadrature *>p.bundle).F_p_R == NULL
        or (<bundle_quadrature *>p.bundle).U_p_L == NULL or (<bundle_quadrature *>p.bundle).U_p_R == NULL
    ):
        return -1
    return 0

cdef void finalize_PRHQ_quadrature(packet_PRHQ * p) noexcept nogil:
    cdef bundle_quadrature * b = <bundle_quadrature *> p.bundle
    if b != NULL:
        if b.t_cf != NULL:
            free(b.t_cf)
...
cdef void covmat_quadrature(void * packet) noexcept nogil:
...
cdef int initialize_PRHQ(
    ...
    p.covmat = covmat_quadrature
    ret_method = initialize_PRHQ_quadrature(p)
    if ret_method !=0:
        return ret_method
    if (
